...
public final class TextUpperPlugin implements AiiaPlugin {
    private static final PluginManifest MANIFEST = new PluginManifest(
            "store.aiia.aiip.text-upper",
            "Text Upper",
            "1.0.0",
            "aiia.store.textupper.TextUpperPlugin",
    ...
    public Object call(String name, JsonObject arguments, Continuation<? super String> continuation) {
        if (!"uppercase".equals(name)) return "error: unknown tool " + name;
        return PluginSupport.execute("uppercase", name, arguments);
    }
}
