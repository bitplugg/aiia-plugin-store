...
public final class ContactMaskPlugin implements AiiaPlugin {
    private static final PluginManifest MANIFEST = new PluginManifest(
            "store.aiia.dex.contact-mask",
            "Contact Mask",
            "1.0.0",
            "aiia.store.contact.ContactMaskPlugin",
    ...
    public List<PluginTool> tools() {
        return Collections.singletonList(new PluginTool(
                "contact_mask",
                "Mask an email or phone-like value",
    ...
    public Object call(String name, JsonObject arguments, Continuation<? super String> continuation) {
        if (!"contact_mask".equals(name)) return "error: unknown tool " + name;
        return PluginSupport.execute("contact_mask", name, arguments);
    }
}
