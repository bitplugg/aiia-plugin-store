...
public final class MarkdownCleanerPlugin implements AiiaPlugin {
    private static final PluginManifest MANIFEST = new PluginManifest(
            "store.aiia.dex.markdown-cleaner",
            "Markdown Cleaner",
            "1.0.0",
            "aiia.store.markdown.MarkdownCleanerPlugin",
    ...
    public Object call(String name, JsonObject arguments, Continuation<? super String> continuation) {
        if (!"markdown_clean".equals(name)) return "error: unknown tool " + name;
        return PluginSupport.execute("markdown_clean", name, arguments);
    }
}
