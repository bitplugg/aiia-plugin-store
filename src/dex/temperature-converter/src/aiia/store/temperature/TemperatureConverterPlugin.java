package aiia.store.temperature;
...
public final class TemperatureConverterPlugin implements AiiaPlugin {
    private static final PluginManifest MANIFEST = new PluginManifest(
            "store.aiia.dex.temperature-converter",
            "Temperature Converter",
            "1.0.0",
            "aiia.store.temperature.TemperatureConverterPlugin",
    ...
    public Object call(String name, JsonObject arguments, Continuation<? super String> continuation) {
        if (!"temperature".equals(name)) return "error: unknown tool " + name;
        return PluginSupport.execute("temperature", name, arguments);
    }
}
